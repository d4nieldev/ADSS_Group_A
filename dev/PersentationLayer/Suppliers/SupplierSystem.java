...
public class SupplierSystem {
    ...
    private static void help() {
        ...
        manual += "                              update payment condition [new_paymentCondition]\n";
    ...
    public static void addSupplier() {
        ...
        switch(type){
            ...
            case 2:{
                ...
                msg = ss.addOnOrderSupplierBaseAgreement(name, phone, bankAccount, fields, paymentCondition, amountTodiscount, names, phones, maxDays);
                System.out.println(msg);
                break;
            }
            case 3:{
                ...
                msg = ss.addSelfPickupSupplierBaseAgreement(name, phone, bankAccount, fields, paymentCondition, amountTodiscount, names, phones, address);
                System.out.println(msg);
                break;
            }
            default:{
    ...
    public static void editSupplier(String[] commandTokens){
        ...
        switch(editCommandTokens[0]){
            case "update": {
                switch(editCommandTokens[1]){
                    case "name": {
                        msg = ss.setSupplierName(supId, editCommandTokens[2]);
                        break;
                    }
                    case "phone": {
                        msg = ss.setSupplierPhone(supId, editCommandTokens[2]);
                        break;
                    }
                    case "bank": {
                        msg = ss.setSupplierBankAccount(supId, editCommandTokens[2]);
                        break;
                    }
                    case "fields": {
                    ...
                    case "payment": {
                        msg = ss.setSupplierPaymentCondition(supId, editCommandTokens[3]);
                        break;
                    }
                    case "amount": {
                        msg = ss.setSupplierAmountToDiscount(supId, makeAmountDiscountMap());
                        break;
                    }
                    default: {
    ...
    /**
     * Adds a new agreement about a specific product with a specific supplier (Updates the agreement if it already exists)
     * @param commandTokens
     */
    public static void addAgreement(String[] commandTokens){
        ...
        String msg = ss.addSupplierProductAgreement(supplierId, productId, productSupplierId, stockAmount, basePrice, amountDiscount, manufacturer);
        System.out.println(msg);
