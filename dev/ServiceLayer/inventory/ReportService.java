...
public class ReportService {
    ...
    public void importInventoryReport(int branchId) throws Exception {
        ...
        for (Integer productCode : productsCode) {
            System.out.printf("%-5d%-20s%-10d%-20d%-20d%n", index, codeToName.get(productCode), productCode,
                    shelfAmount.get(productCode), storageAmount.get(productCode));
    ...
    public void importInventoryReportByCategories(int branchId, List<Integer> categoriesId) throws Exception {
        ...
        int index = 1;
        for (Integer productCode : productsCode) {
            System.out.printf("%-5d%-20s%-20d%-20d%-20d%n", index, codeToName.get(productCode), productCode,
                    shelfAmount.get(productCode), storageAmount.get(productCode));
    ...
    public void importExpiredAndFlawsReport(int branchId) throws Exception {
        ...
        System.out.format("%-10s%-20s%-15s%-20s%-25s%-25s%n", "NO.", "NAME", "Category Name", "Specific ID",
                "FlawDescription", "ExpiredDate");

        // Get all flow and expired products
        Map<Integer, ProductBranch> products = expiredAndFlawReport.getProducts();
        Map<Integer, Map<Integer, LocalDate>> idToExpiredSpecificIdAndDate = expiredAndFlawReport
                .getIdToExpiredSpecificIdAndDate();
        Map<Integer, Map<Integer, String>> codeToSpecificDescription = expiredAndFlawReport
                .getCodeToSpecificDescription();
        ...
        // Iterate through all expired products and print the same details, with
        // "EXPIRED" as the flow description
        for (Integer productCode : products.keySet()) {
            Map<Integer, LocalDate> expiredDate = idToExpiredSpecificIdAndDate.get(productCode);
            for (int id : expiredDate.keySet()) {
                System.out.format("%-10d%-20s%-15s%-20d%-25s%-25s%n", index++, products.get(productCode).getName(),
                        products.get(productCode).getCategoryName(), id, "X", "Expired at: " + expiredDate.get(id));
            }
        }
    ...
    public void importDeficientReport(int branchId) throws Exception {
        ...
        int index = 1;
        for (Integer productCode : products.keySet()) {
            System.out.printf("%-5d%-20s%-20d%-20d%n", index, products.get(productCode).getName(), productCode,
                    idToMissingAmount.get(productCode));
    ...
    public void importProductReport(int branchId, int productCode) throws Exception {
        ...
        System.out.println("===============================================");
        System.out.printf("%-5s%-20s%-20s%-20s%-20s%-20s%n", "NO.", "Product Name", "Code", "Manufacturer",
                "Shop Amount", "Storage Amount");

        System.out.printf("%-5d%-20s%-20d%-20s%-20d%-20d%n", 1, productBranch.getName(), productCode,
                productBranch.getManufacturer(), productBranch.getOnShelfProduct().size(),
